...
public class Util {
	...
	public static int getInt(Context context, String key, String itemId, int defaultVal) {
		...
		Log.i("", "[Test][getInt] key = " + key + " / itemId = " + itemId + " / val = " + val + " / defaultVal = "
				+ defaultVal);
		return val;
	}
	...
	public static void putInt(Context context, String key, String itemId, int inputVal) {
		...
		Log.i("", "[Test][putInt] key = " + key + " / itemId = " + itemId + " / inputVal = " + inputVal);
	}
	...
	public static String getTotalProfit(Context context) {
		...
		for (String code : Util.getStockCodes(context.getApplicationContext())) {
			double eachProfit = Double.parseDouble(getStockProfit(context.getApplicationContext(), code));
	...
	public synchronized static void makeNotification(Context context, String itemId, String title, String summary,
			PendingIntent pIntent) {

		Notification.Builder builder = new Notification.Builder(context).setContentTitle(title).setContentText(summary)
				.setSmallIcon(R.drawable.noti).setContentIntent(pIntent).setAutoCancel(true)
	...
	public static PendingIntent getPendingIntent(Context context, int pendingFlags) {
		...
		PendingIntent operation = PendingIntent.getBroadcast(context, ALARM_REQ_ID, intent, pendingFlags);
	...
	public static void startRepeatAlarm(Context context, long period) {
		...
		Log.i("",
				"[Test][startRepeatAlarm]:: period (millisecond) = " + period + " / Time now : "
	...
	public static boolean getDBUpdateFlag(Context context) {
		SharedPreferences pref = context.getApplicationContext().getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
		return pref.getBoolean("dbupdate", false);

	}

	public static void setDBUpdateFlag(Context context, boolean flag) {
		SharedPreferences pref = context.getApplicationContext().getSharedPreferences(PREF_NAME, Context.MODE_PRIVATE);
		pref.edit().clear();
		pref.edit().putBoolean("dbupdate", flag).commit();
	}
